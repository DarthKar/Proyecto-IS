    ...
    def agregarPlatoConIngredientes(plato, precio, ingredientes):
        ...
        try:
            ...
            for ingrediente, cantidad in ingredientes.items():
                ...
                cursor.execute(
                    "SELECT 1 FROM PlatoIngredientes WHERE plato_nombre = ? AND ingrediente_nombre = ?",
                    (plato, ingrediente)
                )
                if cursor.fetchone() is not None:
                    print(f"Advertencia: La combinación de plato '{plato}' e ingrediente '{ingrediente}' ya existe.")
                    continue  # O usa 'return' si prefieres detener la inserción en caso de duplicado
